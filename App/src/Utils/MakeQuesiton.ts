...
function PlaygorundLogic(formData: any) {
    const nums = [
        `+ ${Math.floor(
            Math.random() * 10 ** parseInt(formData.numberOfDigits),
        ).toString()}`,
    ];
...
function MultiplicationLogic(formData: any) {
    const multiplicand = Math.floor(
        Math.random() * 10 ** parseInt(formData.numberOfdigitMultiplicand),
    );
    const multiplier = Math.floor(
        Math.random() * 10 ** parseInt(formData.numberOfdigitMultiplier),
    );
...
function DivisionLogic(formData: any) {
    ...
    for (let i = 0; i < numberOfQuestions; i++) {
        ...
        const minMultiple = Math.ceil(
            10 ** (numberOfdigitDividend - 1) / divisor,
        );
        ...
        do {
            ...
            if (attempts > 100) {
                console.error(
                    'Unable to find valid dividend after 100 attempts. Skipping question.',
                );
                break;
            }
